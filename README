...
Once you have the certificate from Apple for your application, export your developer key
and the apple push notification development certificate as p12 files. Here is a quick walkthrough on how to do this:

1. Click the disclosure arrow next to your certificate in Keychain Access and select the certificate and the developer key
...
Do the same steps for your production certificate
...
==One App Push Notifications Usage

  $ ./script/console
  >> app = APN::App.create(:name => "My App", 
      :apn_dev_cert => "PASTE THE CONTENTS OF THE 'apn_development.pem' file here", 
      :apn_prod_cert => "PASTE THE CONTENTS OF THE 'apn_production.pem' file here", 
...
APN on Rails has the following default configurations that you change as you see fit:
  ...
  configatron.apn.cert #=> File.join(RAILS_ROOT, 'config', 'apn_development.pem')
  
  # production (delivery):
  configatron.apn.host # => 'gateway.push.apple.com'
  configatron.apn.cert #=> File.join(RAILS_ROOT, 'config', 'apn_production.pem')
  
  # development (feedback):
  configatron.apn.feedback.passphrase # => ''
  configatron.apn.feedback.port # => 2196
  configatron.apn.feedback.host # => 'feedback.sandbox.push.apple.com'
  configatron.apn.feedback.cert #=> File.join(RAILS_ROOT, 'config', 'apn_development.pem')
  
  # production (feedback):
  configatron.apn.feedback.host # => 'feedback.push.apple.com'
  configatron.apn.feedback.cert #=> File.join(RAILS_ROOT, 'config', 'apn_production.pem')
